...
function CategoryManager() {
    ...
    const handleSubmit = async (e) => {
        ...
        const confirmResult = await Swal.fire({
            title: modalType === 'add' ? 'Xác nhận thêm danh mục' : 'Xác nhận chỉnh sửa danh mục',
            text: `Bạn có chắc chắn muốn ${modalType === 'add' ? 'thêm' : 'lưu thay đổi cho'} danh mục này?`,
            icon: 'question',
                                ...
                                {categories.length === 0 ? (
                                    ...
                                    categories.map((cat, index) => (
                                        <tr key={cat.id} className="hover:bg-gray-50 transition-all duration-200">
                                            ...
                                            <td className="p-4 border-t border-gray-200">{cat.name}</td>
