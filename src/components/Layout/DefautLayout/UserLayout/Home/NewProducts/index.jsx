    ...
    useEffect(() => {
        const fetchNewProducts = async () => {
            try {
                ...
                // Lọc sản phẩm thuộc danh mục "New" hoặc mới ra mắt (dựa trên createdAt hoặc isNew)
                const filteredProducts = products
                    .filter(product =>
                        product.categoryName?.toLowerCase() === 'new'
                    )
                    .map(product => ({
            ...
            <div className="w-[96%] mx-auto px-4">
                ...
                {optimizedNewProducts.length === 0 ? (
                    <div className="text-center py-16">
                        ...
                        <p className="text-gray-500 text-lg">Không có sản phẩm mới nào hiện tại.</p>
                        <Link
                            to="/menu"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {optimizedNewProducts.map((product) => (
                            <motion.div
                                key={product.id}
