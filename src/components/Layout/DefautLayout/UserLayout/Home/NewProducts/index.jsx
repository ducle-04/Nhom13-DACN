...
const NewProducts = () => {
    ...
    useEffect(() => {
        const fetchNewProducts = async () => {
            try {
                ...
                // Lọc sản phẩm thuộc danh mục "New"
                const filteredProducts = products
                    .filter(product => product.categoryName?.toLowerCase() === 'new')
                    .map(product => {
                ...
                if (err.response?.status === 401) {
                    setError('Vui lòng đăng nhập để xem sản phẩm mới.');
                    navigate('/login');
                } else {
    ...
    const optimizedNewProducts = useMemo(() => newProducts, [newProducts]);
            ...
            <div className="min-h-screen bg-gradient-to-br from-slate-50 via-orange-50 to-amber-50 flex items-center justify-center">
                ...
                >
                    ...
                    <div className="backdrop-blur-sm bg-white/20 rounded-2xl p-6 shadow-xl">
                        <p className="text-slate-700 text-xl font-medium">Đang tải sản phẩm mới...</p>
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6">
                ...
                >
                    ...
                    <h2 className="text-7xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-600 via-amber-500 to-yellow-500 mb-8 font-montserrat tracking-tight leading-tight">
                        Sản Phẩm Mới FoodieHub
                    </h2>
                    <div className="max-w-4xl mx-auto">
                        <p className="text-slate-700 text-2xl leading-relaxed mb-6">
                            Khám phá những món ăn mới nhất từ FoodieHub
                        </p>
                        <p className="text-slate-500 text-lg leading-relaxed">
                            Thưởng thức thực đơn mới ra mắt – chuẩn vị, hấp dẫn ngay từ lần thử đầu tiên!
                        </p>
                    </div>
                </motion.div>

                {optimizedNewProducts.length === 0 ? (
                    <motion.div
                    ...
                    >
                        ...
                        <div className="backdrop-blur-sm bg-white/60 rounded-3xl p-10 max-w-md mx-auto shadow-xl border border-gray-100">
                            <h3 className="text-2xl font-bold text-gray-800 mb-4">Chưa có sản phẩm mới</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                        {optimizedNewProducts.map((product, index) => (
                            <motion.div
                                key={product.id}
